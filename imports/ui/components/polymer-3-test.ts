...
export class MyApp extends AdornisMongoMixin(Element) {
    // eslint-disable-line
    ...
    public handleUpdateMessage() {
        this.$.mdata2.insert({
            usage: Math.random() < 0.5 ? "wat anderes" : "useful usage",
        });
    }

    private _toggleParams(e) {
        this.set("subParams2", e.detail.value ? [] : ["useful usage"]);
    }

    private _add() {
        this.$.mdata1.insert({
            name: "hihihi" + Math.floor(Math.random() * 10),
        });
    }

    private _switchFilter(e) {
        this.filter = !e.detail.value ? {} : { name: "hihihi" };
    }

    private _computeTest(route) {
        this.log("hallo welt");
        return "hallo welt";
    }
}
