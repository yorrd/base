...
class MongoBind extends AdornisMongoMixin(Element) {
                    ...
                    return function update(newValue, diff) {
                        ...
                        if (diff) {
                            ...
                            if (forbiddenKeys.includes(key))
                                throw new Error(
                                    "cant update a field which is used in the selector",
                                );
                        ...
                        if (newValue._id)
                            this.getCollection(this.collection).update(
                                { _id: newValue._id },
                                { $set: setObj },
                            );
                        else
                            throw new Error(
                                "should never get here, creation should happen at the bottom of the file",
                            );
                    };
                },
            },
    ...
    public _collectionChanged(undefinedOrSubStatus) {
        if (undefinedOrSubStatus === false) {
            return;
        } // if undefined, this has been called by a different observer
        if (!this.selector || !this.collection) {
            return;
        }
        if (undefinedOrSubStatus === undefined) {
            // means that we actually changed something so that we need to resubscribe
    ...
    public _updateResults(justInsertedDefaultWithThisId?: number) {
        const didIWatchBefore = this.watch;
        this.watch = false;

        const result = this.getCollection(this.collection).findOne(
            this.selector,
        );
        ...
        if (result) {
            ...
            if (justInsertedDefaultWithThisId) {
                throw new Error(
                    "Your default case is inserting objects which dont satisfy your filter. " +
            ...
            if (!this.createNew) {
                console.error(
                    "You dont want me to create a new element (this.createNew) " +
                        "and there is nothing in the collection",
                );
                console.info(
                    "This could be because youre setting the wrong sub-params (temporarily?)",
                );
                return;
            }
