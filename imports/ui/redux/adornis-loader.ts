...
export class AdornisLoader extends Element {
...
<div id="loaderWrapper" style$="background: {{background}}" hidden>
    ...
    public connectedCallback() {
        ...
        if (isNaN(+this.timeOut)) {
            console.error("AdornisLoader::timeOut only accepts numbers!");
            return;
        }
        this.show();
    }
    ...
    public show() {
        this._fadeIn();
        if (+this.timeOut > 0) {
            setTimeout(this._fadeOut.bind(this), +this.timeOut);
        }
    }
    ...
    public _fadeIn() {
        if (this.fadeIn) {
            this.$.loaderWrapper.setAttribute("class", "fadein");
        }
